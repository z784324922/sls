...
-   函数示例：将高级参数中获取信息重新赋值给local。
    ...
    ![高级参数](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/1267159/156862390356312_zh-CN.png)
...
-   参数说明
    ...
    |refresh\_interval\_max|int|否|默认值为60，单位为秒。加工任务失败时，重试请求的间隔最大时间，建议使用默认值。|
    |use\_ssl|Bool|否|是否使用ssl协议进行安全连接，默认值为false。 **说明：** 这里使用的是本地的ca证书进行的验证，目前暂时不支持使用自己的ca证书进行双向连接，如果对数据安全有严格要求，请务必使用`use_ssl=true`配置。
...
-   函数格式
    ...
    res_log_logstore_pull(endpoint, ak_id, ak_secret, project, logstore, fields, from_time="begin", to_time="end", fetch_include_data=None, fetch_exclude_data=None, primary_keys=None, upsert_data=None, delete_data=None,refresh_interval_max=60,fetch_interval=2)
    ```
 ...
 **说明：** 如果填入的是一个未来时间，只会将该Logstore所有数据拉取完毕，并不会开启持续拉取任务。
 ...
 |
    ...
    |fetch\_exclude\_data|搜索字符串|否|字符串黑名单机制，必须满足`fetch_exclude_data`才会丢弃，否则保留。|
    |primary\_keys|字符串列表|否|维护表格时的关键字段列表。如果`fields`中对关键字进行修改，这里应使用修改后的字段名，将修改后的字段作为主键。 **说明：** `primary_keys`字段必须存在于`fields`字段中，并且仅支持要拉取数据的目标Logstore中只有一个Shard的情况，多个Shard暂时不支持。

 |
    |fetch\_interval|Int|否|开启持续提取任务时，每次提取请求的时间间隔，默认为2秒。该值设置时必须大于或者等于1秒。|
    |delete\_data|搜索字符串|否|对满足条件且配置了`primary_keys`的数据，在表格中进行删除操作。更多条件匹配请参见[查询字符串语法](cn.zh-CN/.md#)。|
    |refresh\_interval\_max|Int|否|默认值为60，单位为秒。加工任务失败时，重试请求的间隔最大时间，建议使用默认值。|
...
-   参数说明
 ...
 |
    ...
    |fetch\_interval|String|否|提取失败后重新提取的时间间隔，默认为2秒。|
    |refresh\_retry\_max|String|否|提取失败后重新提取的最大时间间隔，默认60秒。|
