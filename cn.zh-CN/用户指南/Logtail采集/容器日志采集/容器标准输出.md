# 容器标准输出 {#concept_yk2_t2d_wdb .concept}
...
-   **数据处理**。采集到的数据默认字段为`content`，支持通用的处理配置。请参考[简介](intl.zh-CN/用户指南/Logtail采集/自定义插件/简介.md)配置一种或多种采集方式。
...
**说明：** Logtail支持对采集的数据进行处理加工后上传，处理方式参见[处理采集数据](intl.zh-CN/用户指南/Logtail采集/自定义插件/处理采集数据.md)。
...
Logtail对于采集到的Docker标准输出，支持[通用数据处理方式](intl.zh-CN/用户指南/Logtail采集/自定义插件/简介.md)。建议使用基于上一节的多行日志格式，使用正则表达式将日志解析成time、module、thread、class、info。
