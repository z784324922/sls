...
|
|[计量日志](#)|Logtail定时上报的状态日志。每分钟记录一次。指定查询计量日志时，需要在查询语句中指定`__topic__: logtail_status`。|
|[Logtail状态日志](#)|用户计量相关统计。Logstore粒度的存储空间，读写流量，索引流量，请求次数等统计信息。每小时统计一次。指定查询Logtail状态日志时，需要在查询语句中指定`__topic__: metering`。
...
|process\_queue\_full|满了的处理队列数量。|​1|
