...
3.  对比心跳状态为**OK**的节点数是否和集群中Worker节点数一致。根据对比结果选择排查方式。
    1.  机器组中所有节点的心跳状态均为**Failed**。
        ...
        -   若您使用[自建Kubernetes安装方式](intl.zh-CN/用户指南/Logtail采集/安装/Kubernetes日志采集.md#section_kdx_bqr_zdb)，请参考[ZH-CN\_TP\_13059.md\#table\_op1\_2qr\_zdb](intl.zh-CN/用户指南/Logtail采集/安装/Kubernetes日志采集.md#table_op1_2qr_zdb)，检查\{your-project-suffix\}、\{regionId\}、\{aliuid\}、\{access-key-id\}和\{access-key-secret\}是否已正确填写，若填写错误请执行命令`helm del --purge alibaba-log-controller`删除安装包，并重新安装。
    2.   机器组心跳数少的节点于集群中Worker节点数。
