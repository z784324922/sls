...
-   **[分析语句](intl.zh-CN/用户指南/查询与分析/实时分析简介.md)**：使用SQL对查询结果进行分析，获取统计结果。通过SQL的[limit语法](intl.zh-CN/用户指南/查询与分析/分析语法与函数/LIMIT语法.md)来达到分页效果。
...
为了解决SQL分页的问题，日志服务提供了标准SQL的[limit分页语法](intl.zh-CN/用户指南/查询与分析/分析语法与函数/LIMIT语法.md)：
